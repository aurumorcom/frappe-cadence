...
from frappe_listmonk.schemas.list import (
	ListCreateRequest,
	ListResponse,
	ListUpdateRequest,
)
...
class ListmonkClient:
	...
	def update_subscriber(
		self,
		subscriber_id: int,
		req: SubscriberUpdateRequest | dict[str, Any],
		method: str = "PATCH",
	) -> SubscriberResponse:
		data = _dump_model(req, exclude_unset=True)
		res = self._request(method, f"/api/subscribers/{subscriber_id}", payload=data)
		return _validate_model(SubscriberResponse, res)
	...
	def update_campaign_subscriber(
		self, campaign_id: int, subscriber_id: int, status: str = "scheduled"
	) -> dict[str, Any]:
		payload = {"status": status}
		res = self._request(
			"POST", f"/api/campaigns/{campaign_id}/subscribers/{subscriber_id}", payload=payload
		)
	...
	def find_list_by_name(self, name: str) -> dict[str, Any] | None:
		try:
			res = self._request("GET", "/api/lists", params={"query": name})
			items = (
				res if isinstance(res, list) else (res.get("results", []) if isinstance(res, dict) else [])
			)
			for item in items:
				if isinstance(item, dict) and item.get("name") == name:
					return item
		except Exception as exc:
			frappe.logger("listmonk").warning(f"Failed to lookup list by name '{name}': {exc}")
		return None
