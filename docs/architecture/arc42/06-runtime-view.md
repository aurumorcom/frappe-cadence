...
7. **Delivery & Engagement Tracking**: Providers dispatch the message. As engagement occurs (replies, bounces, unsubscribes), the `cadence_provider.report_event` webhook updates the MCC state to terminal (`Completed`, `Error`, `Unsubscribed`), emitting events that cancel further step processing.
