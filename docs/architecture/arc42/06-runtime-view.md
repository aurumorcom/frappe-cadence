...
2. The system enqueues [`sync_all_crm_leads()`](apps/frappe_cadence/frappe_cadence/integrations/listmonk/jobs/contact.py:59) to query all CRM leads.
3. Asynchronous [`upsert_contact()`](apps/frappe_cadence/frappe_cadence/integrations/listmonk/jobs/contact.py:15) jobs synchronize subscriber records and store `listmonk_id` back onto each `CRM Lead`.
...
2. `crm_lead.on_update` enqueues [`evaluate_cadences_for_lead()`](apps/frappe_cadence/frappe_cadence/jobs/cadence.py:107) and [`upsert_contact()`](apps/frappe_cadence/frappe_cadence/integrations/listmonk/jobs/contact.py:15).
...
4. The completion hook updates [`Multi Channel Cadence`](apps/frappe_cadence/frappe_cadence/cadence/doctype/multi_channel_cadence/multi_channel_cadence.py:7) to `Provisioning` and triggers [`add_contact_to_sequence()`](apps/frappe_cadence/frappe_cadence/jobs/multi_channel_cadence.py:25).
5. [`add_contact_to_sequence()`](apps/frappe_cadence/frappe_cadence/jobs/multi_channel_cadence.py:25) fetches sender [`User Bio`](apps/frappe_cadence/frappe_cadence/cadence/doctype/user_bio/user_bio.py:8) and research context, formats template attributes, attaches the subscriber to the Listmonk campaign sequence, and sets status to `Scheduled`.
...
3. Valid payloads are parsed in [`process_webhook_payload()`](apps/frappe_cadence/frappe_cadence/integrations/listmonk/jobs/webhook.py:98), matching `Multi Channel Cadence` by `listmonk_contact_id` and `listmonk_sequence_id`.
4. Transitions status according to event types:
   - `campaign.started` / `sequence.step_executed` $\rightarrow$ `In Progress`
   - `replied` $\rightarrow$ `Replied` (and triggers [`remove_contact_from_sequence()`](apps/frappe_cadence/frappe_cadence/jobs/multi_channel_cadence.py:99))
