...
class TestMultiChannelCadenceJobs(unittest.TestCase):
	...
	def test_add_contact_to_sequence(
		self,
	...
	@patch("frappe_cadence.jobs.multi_channel_cadence.ensure_listmonk_authorized")
	@patch("frappe_cadence.jobs.multi_channel_cadence.ListmonkClient")
	def test_remove_contact_from_sequence(self, mock_client_cls: MagicMock, mock_auth: MagicMock) -> None:
		client_inst = MagicMock()
		mock_client_cls.return_value = client_inst

		remove_contact_from_sequence("MCC-001", listmonk_contact_id=10, listmonk_sequence_id=5)
