...
class TestCRMLeadToMCCFlowE2E(FrappeTestCase):
	...
	def test_new_crm_lead_syncs_contact_and_triggers_mcc_creation(self, mock_ensure_auth, mock_create_contact) -> None:
		lead = frappe.get_doc({
			"doctype": "CRM Lead",
		...
		mccs = frappe.get_all("Multi Channel Cadence", filters={"recipient": lead.name, "cadence_name": self.cadence.name})
