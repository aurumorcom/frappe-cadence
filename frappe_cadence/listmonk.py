...
def webhook() -> dict:
	...
	secret = frappe.conf.get("listmonk_webhook_secret") or (settings.get_password("webhook_secret") if settings else "")
	...
	contact_id = data.get("subscriber_id") or data.get("contact_id") or data.get("id") or payload.get("contact_id") or payload.get("subscriber_id")
		...
		for mcc_item in mcc_list:
			...
			if not recipient_email and mcc_doc.recipient:
				recipient_email = frappe.db.get_value(
					getattr(mcc_doc, "cadence_for", None) or "CRM Lead",
					mcc_doc.recipient,
					"email_id",
			...
			if target_status in ["In Progress", "Replied", "Finished"]:
				frappe.get_doc({
					...
					"content": data.get("body") or data.get("content") or f"Executed step for Listmonk event {event_type}",
					"reference_doctype": "Multi Channel Cadence",
					"reference_name": mcc_doc.name,
					"sender": data.get("sender_email") or payload.get("sender_email") or "listmonk@system.local",
