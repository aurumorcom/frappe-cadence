...
| **Terminal State Interruption** | A lead replies to an email while the sequence is sleeping before the next step. | `cadence_provider.report_event` Webhook | The MCC state is updated to `Completed`, and an `mcc_completed` event is fired to terminate the sleeping orchestration thread immediately. |
